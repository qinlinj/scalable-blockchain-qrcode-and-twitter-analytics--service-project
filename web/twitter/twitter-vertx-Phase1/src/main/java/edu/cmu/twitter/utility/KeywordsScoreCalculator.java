...
public class KeywordsScoreCalculator {
    ...
    public Map<Long, Double> calculateKeywordsScores(
            Set<Tweet> requestUserTweets, Map<Long, List<Tweet>> userTweetsMap,
        ...
        for (Long otherUserId : userTweetsMap.keySet()) {
            ...
            for (Tweet tweet : otherUserTweets) {
                if (matchesTweetType(tweet, type)) {
    ...
    private boolean matchesTweetType(Tweet tweet, String type) {
        return "both".equalsIgnoreCase(type) ||
                ("reply".equalsIgnoreCase(type) && tweet.getIn_reply_to_user_id() != null) ||
                ("retweet".equalsIgnoreCase(type) && tweet.getRetweeted_status_user_id() != null);
    }
